...
Disse repository bevat eign keyboard layouts veur een Planck ortholinear keyboard.

Alle mappen hieronder maakt gebroek van krek dezelfde hardwarebasis (Planck v6, 48-key layout):

* **Franken-Planck:** Een 9-laags indeling met de volgende functionaliteiten:
  * `hjkl` cursor-toetsen (Vim-stiel) under de Lower-laag (ingedrukt hoolden gef herhalende pieltjes).
  * Home-row modifiers.
  ...
  * Directe toegang tot tilde, backtick en quotes op laag 0 (daor goa'j strakkes met langs de layer-tap restricties).
  * Ergonomische doambediening: Tikken op Raise gef Backspace, tikken op Lower gef Delete.
...
De mappen bint zo inricht dat de QMK-compiler de bestanden direct kan mappen op de hoofd-repository. De PDF-bestanden met grafische lay-outs stoat in de hoofdmap:
...
Umdat disse repository buten de standaard QMK-mappenstructuur steet, mu'j de CLI eerst vertellen waor disse map te vinden is veurda'j gaot compileren.
...
Gebroek de standaard compileer-opdrachten. QMK pakt automatisch de .json-bestanden op om de firmware te genereren:
